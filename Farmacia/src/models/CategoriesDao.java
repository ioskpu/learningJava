...
public class CategoriesDao {
    ...
    public List listCategoryQuery(String value){
        ...
        String query_search_category = "SELECT * FROM categories WHER name LIKE '%" + value + "%'";
        try {
            conn = cn.getConnection();
    ...
    public boolean updateCategoryQuery(Categories category){
        String query = "UPDATE categories SET name = ?, updated = ?, id = ?";
        Timestamp datetime = new Timestamp(new Date().getTime());
        try{
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
    ...
    public boolean deleteCategoryQuery(int id){
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "No puede eliminar una categoría que tenga relacion con otra tabla");
            return false;
        }
    }
}
